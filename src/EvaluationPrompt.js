...
function save (e) {
  ...
  return (
    <div>
     ...
     {item.choices && (item.choices.length > 0) && <div><RadioControl className="radio-mode" options={item.choices} selected={choice} onChange={(value) => { setChoice(value); setResponses((prev) => {let newd = [...prev]; newd[promptindex] = value; return newd }); }}  /></div>}
