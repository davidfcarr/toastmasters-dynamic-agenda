...
export function EventDateTime(props) {
    ...
    return (
        ...
        <input type="time" value={metadata._rsvp_end_date.replace(/[0-9]{4}-[0-9]{2}-[0-9]{2} /,'')} onChange={(e) => {let value = metadata._rsvp_dates.replace(/[0-9]{2}:[0-9]{2}:[0-9]{2}/,'')+e.target.value; metaMutate( {'post_id':post_id,'kv':[{'key':'_rsvp_end_date','value':value}]}); makeNotification('new date '+value)} } />        
